...
- **Single value per category as magnitude is a bar** - filled length is the cue, the zero baseline anchors it. For multiple values per category, choose grouped bars, suitable facets, or a table; do not substitute categorical dots or lollipops. A dumbbell is valid when the task is comparing two values within each category.
...
- **Scatter requires two quantitative axes.** This is Karthik's form constraint: Unconnected category × quantity dot plots are not allowed. Lollipops are also excluded: use bars for magnitude. Dumbbells are allowed for paired comparisons, with the connection carrying the comparison between two values per category. Renaming a categorical dot plot, numerically coding categories, or supplying legends and shapes does not make it a quantitative × quantitative scatter. Choose bars or a table for categorical magnitude comparisons; a slopegraph remains an option for comparing states. Judge the encoded variables, not the plotting primitive: points on a quantitative relationship or temporal trajectory are not categorical dot comparisons.
