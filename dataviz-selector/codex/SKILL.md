...
- Many comparable series: small multiples or cluster prototypes; avoid spaghetti. Recommending small multiples is not complete until you specify the layout: wrap the panels into a grid (rows x columns) sized to the delivery medium, with the grid's proportions tracking the delivery frame's aspect ratio - a wide frame takes more columns than rows, a tall frame more rows than columns - and a panel count that keeps each panel legible at delivery size; when the panels cannot stay legible, reduce their number or move to a taller medium rather than shrinking them. Never a single-column or single-row strip - a long stack of many panels destroys the side-by-side comparison that motivated the form. State the panel ordering (by magnitude, cluster, or another meaningful key, not source/input order), whether scales are shared or free, and require free scales to be marked per panel.
