...
- Multiple comparable series: default to a single panel with direct-labelled lines. Move to small multiples only when the single panel fails - too many series to direct-label, or the lines overplot into spaghetti. Series count and overplotting are the trigger; the lines crossing or a noisy shape is not, and does not by itself justify faceting. Once faceting is warranted, recommending small multiples or cluster prototypes is not complete until you specify the layout: wrap the panels into a grid (rows x columns) sized to the delivery medium, with the grid's proportions tracking the delivery frame's aspect ratio - a wide frame takes more columns than rows, a tall frame more rows than columns - and a panel count that keeps each panel legible at delivery size; when the panels cannot stay legible, reduce their number or move to a taller medium rather than shrinking them. Never a single-column or single-row strip - a long stack of many panels destroys the side-by-side comparison that motivated the form. State the panel ordering: lay panels in decreasing order of each panel's peak value, or in decreasing order of the story's importance (or another meaningful key such as cluster) - never source/input order - so the reader's eye runs from most to least across the grid, top-left to bottom-right. When the most important panel is not the largest, keep the magnitude order and mark that panel with a heavier line weight rather than moving it out of order. State whether scales are shared or free - free per-panel scales when the message is each panel's shape or turning points and magnitudes differ enough that a shared scale flattens the small series, a shared scale only when comparing levels across panels is the claim - and require free scales to be marked per panel.
- Smoothing a dense series: when a single series carries so many data points that connecting them all buries the trend in jaggedness, do not draw the raw connect-the-dots line as the message. Either overlay a smoothed trend (loess or regression) on the faint raw series, or drop the connecting line entirely and show the points as a scatter with the smoothed trend through them. This is driven by point density within one series, unrelated to how many series there are or to the faceting decision; a series with few points does not need it. Keep at most one or two smoothed lines per graph. Show the smoother's uncertainty, do not let the smoother erase real turning points or extremes the claim depends on, and mark any extrapolation beyond the data as a projection.
...
- Ranking: sorted horizontal bars, longest bar at the top and descending - the ordering carries the ranking, so never leave horizontal bars in input or alphabetical order. (Order matters less for vertical bars read left-to-right, but still order them by a meaningful key.) Bar axis starts at 0; highlight an item only when the question, evidence, or stated story makes it focal. Otherwise keep equal-status items neutral.
- An intrinsically ordered category axis - time (quarters, months, years), stages, sizes, ranked bins - is not a ranking to sort by magnitude, and its reading direction is fixed by the sequence: it runs left-to-right on a horizontal axis, or top-to-bottom when the layout is vertical, never inverted so the sequence climbs bottom-to-top. Sort by magnitude and lay categories down the y-axis only for nominal (unordered) categories; applying the ranked-horizontal-bar layout to a time or sequence axis reads a series as a ranking and stands time on its head. This holds inside small multiples too - every panel keeps the same ordered axis in the same direction.
- A single value per category read as magnitude is a bar: the filled length is the magnitude cue and the zero baseline anchors it. A lollipop or dot-with-stem swaps that filled length for a dot at the end of a hairline - a weaker magnitude read - so choose it only when bars would be too heavy or dense to compare (many categories) and the exact endpoint matters more than the filled quantity; with few categories and a meaningful zero, use bars.
- Composition/share substitution: use 100% stacked bars or areas only when broad mix is the story. Only segments that begin or end on an aligned baseline support precise visual comparison; a fixed-total stack aligns both outer edges, while internal segments still float. If the claim depends on component patterns across periods or groups, use small multiples, grouped bars, lines, or a compact table instead. That a dataset is compositional - its parts sum to a fixed total (100%) - is a property of the numbers, not a reason to choose a composition form; it constrains the data, it does not name the claim. Decide the form from what the claim asks the reader to read: if the claim describes how individual components move across periods or groups (one rises, another falls, a third holds), that is a set of component trajectories - use lines, slopes, small multiples, grouped bars, or a table - even when the shares total 100%. Reserve stacked bars or areas for when the reader's task is genuinely reading one period's mix. Treating "the parts sum to 100%" as support for a stack inverts a data property into a form choice.
...
- Let perception group and link, not just colour. The reader groups by proximity and similarity, and links by connection, before consulting a legend: prefer directly labelled and connected forms (lines, slopes) to a colour the eye must match; use separate panels, spacing, or a light enclosure to separate or mark groups; and keep one focal element as figure against muted ground. Do not hand unrelated series a similar encoding, and do not make several channels compete to be seen first. Choosing the form is also choosing what the reader sees first: pick the form that lets one focal element carry the composition, since the post-render composition gate (`dataviz-aesthetic`) will check exactly that.
