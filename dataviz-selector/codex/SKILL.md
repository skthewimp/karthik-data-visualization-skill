...
- **Distribution/skew/tails:** histogram, density, ECDF, box, or violin. When positive values span many orders of magnitude (income, wealth, sizes, power-law data), a log axis stops the large values saturating the scale and crushing the small ones - decide it with the axis-transform guardrail below, not by reflex.
...
- **Log vs linear axis is a computed decision you own.** For a continuous *position* axis (points, lines, dots, box/violin - not bars/area, which encode by length and need a true zero), call `recommend_scale_transform(values, encoding)`: it computes the positive dynamic range and skew and returns a graded recommendation. Treat it as one input, not a verdict - keep linear when the prompt wants absolute magnitudes, the audience won't read a log axis, or it would mislead, and switch the mark before the scale if a length encoding is what needs the spread. Record the chosen scale in the plan (`scale: linear|log`) and, whenever you go against the tool, a one-line reason - the same way brand overrides a colour recommendation. If you keep log, mark the axis as log and leave tick labels in untransformed units. Log10 needs strictly positive values; the tool refuses otherwise rather than reaching for symlog blind.
