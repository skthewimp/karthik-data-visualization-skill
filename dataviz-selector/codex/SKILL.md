...
- Many comparable series: small multiples or cluster prototypes; avoid spaghetti. Recommending small multiples is not complete until you specify the layout: wrap the panels into a grid (rows x columns) sized to the delivery medium, never a single-column or single-row strip - a long stack of many panels destroys the side-by-side comparison that motivated the form. State the panel ordering (by magnitude, cluster, or another meaningful key, not source/input order), whether scales are shared or free, and require free scales to be marked per panel.
