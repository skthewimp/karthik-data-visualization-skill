...
- **Two-point comparisons:** choose among slopegraphs, paired bars, or tables by whether the task is seeing change, comparing endpoints, or reading exact values. Number of time points alone doesn't fix the form.
...
- **Single value per category as magnitude is a bar** - filled length is the cue, the zero baseline anchors it. For multiple values per category, choose grouped bars, suitable facets, or a table; do not substitute categorical dots, lollipops, or dumbbells.
...
- **Scatter requires two quantitative axes.** This is Karthik's form constraint: category × quantity point comparisons are not allowed, including horizontal dot plots. Renaming the form, numerically coding categories, adding stems/connections, or supplying legends and shapes does not satisfy it. Choose bars or a table for categorical magnitude comparisons; a slopegraph remains an option for comparing states. Judge the encoded variables, not the plotting primitive: points on a quantitative relationship or temporal trajectory are not categorical dot comparisons.
