...
- Series/category and on-mark data labels use a font-relative readable measure (`max_label_chars_per_line`, default 24) and at most `max_label_lines` (default 3), rather than the annotation allowance of 32% of canvas width. If a label exceeds that budget, `wrapped_text` is ellipsized and the result returns `curtailed: true` plus intact `full_text`; the builder must place the full name in a compact key/footnote or choose a form that can carry it.
