...
## Forward data (prepare the frame before render)

A weak build model that writes the whole chart also gets the settled mechanics wrong - tiny fonts, an unplanned axis title, a value axis the direct labels already replace, `limits = c(0, 100)` on a 1-44 line, a hand-picked hue, one tick per month. Flagging these after the render gets ignored, and rewriting the model's code afterwards ships code that contradicts itself, so the source is split by construction. `scaffold_chart` writes everything decidable from the plan and the tool results (data typing, `fmt_value`, palette, value and date scales (a sparse date axis ticks only at the data's own dates), facet grid, titles, fonts, margin, theme, legend or subtitle key, value axis dropped once `value_labels` carry the reading) and leaves one `chart_marks` slot; its scales, labs and theme come after the slot, so an override there loses. The scaffold also carries three ggplot2 layers that apply the placement rules when drawn, from the drawn geometry and each label's glyphs: `bar_values()` (a bar's value inside its end in the ink that reads on its fill, or past the end when the bar is too short - through dodge, stack and flip) `end_labels()` (line names past the last points, wrapped to a capped band, spread apart with leaders when they would overlap) and `point_labels()` (values beside their points - toward the panel's middle, then above, below, the diagonals, the outer side - at the first spot clear of the drawn lines, markers, other labels and the panel edge; unlabelled rows of the line count as its path). `check_chart` restores edited scaffold regions and checks the slot on the built plot object laid out at the delivery size - build errors, non-layers, `geom_label`, off-palette colours, undersized text, a label on another observation's bar or far from its own mark (matched by observation, never by a number parsed from the label), text that fails contrast on the surface actually behind it (a fill, alpha included, or the page), a backdrop layer that stretches the value axis, hand-set text sitting on a drawn line or segment (`TEXT_ON_MARK`, fixed through `point_labels()`), marks that do not spread along the value axis, missing promised labels - returning a numbered `fix_list`. The frame decides what a mark is: an interval frame draws each mark between its ends, a label measure prints through its own `fmt_<name>()` (on bars as a `bar_values()` note past the end, clear of the value), and panel groups that name their categories become one native plot per region (`chart_regions()`) under one page frame, with their boxes returned for a compositor. Routing words are read leniently, and a source with no scaffold record is reported as `UNSCAFFOLDED_BUILD` rather than refused. `dataviz_mcp/scaffold.py`.

A weak build model that hand-builds its dataframe inline in the chart code reverses category/value associations and lets an internal helper column leak in as a plotted series - determinism failures, so they are removed in code, not warned against. `prepare_plot_data` takes a role map decided at selection (which source column is the category/x, the value, the optional series and facet, plus canonical orders and an aggregate for repeated keys) - `value` may be a list of value columns for a wide frame (one per series), each melted into a series named by the column, so a dense multi-series chart runs the same mechanical path instead of a hand-built inline frame and the data - a `dataset_path` for dataset-to-story, or the recovered `columns`/`rows` a repair already carries - and emits one tidy long-format frame. It keeps **only** the mapped columns, so a helper column cannot become a series, and writes an explicit `order` column pinning one canonical order shared by every mark and label, so a stamped value cannot drift onto the wrong category. A duplicate `(category, series, facet)` key with no aggregate is an error, never a silent pick. A mark drawn between two numbers maps `start`/`end` (a missing end stays blank, never derived); a number printed beside a mark but not drawn maps under `labels`, becoming its own column on the observation's row in its own units - never a series, never on the value scale. `default_plot_data_map` reads a role map off the table itself (mostly-numeric columns are measures; the first is the value, the rest label measures) for a harness whose plan returned no usable map. Build loads the returned `plot-data.csv` and plots it verbatim, reshaping nothing; the resolution is the fourth of the decide-at-select / resolve-by-tool / apply-at-build split beside colour, precision, and the frame. `dataviz_mcp/plot_data.py`.
...
- `recommend_table_layout` measures formatted headers/cells with grid/ragg font metrics,
  ...
  the tool resolves bars, shading, sparklines and emphasis into per-cell styles that
  `render_table_from_plan` draws, and inspection flags `TREATMENT_NOT_DRAWN`.
  See [table tool inputs and example](../dataviz_mcp/README.md#recommend_table_layout).
- `recommend_layout` returns `width_px x height_px x dpi`, a facet grid, x-label rotation, and reserved title/subtitle/footer bands from the chart's shape expressed as counts (`x_slots` / `y_slots`, `filled_marks`, `n_panels`, line counts). For filled marks it also returns a reasoned `bar_orientation` (`vertical`/`horizontal`) with a `bar_orientation_reason`, computed from longest-label width vs slot - short labels that sit flat keep bars vertical, labels too wide for their slot go horizontal (rows) - and a `recommended_data_label_pt` sized to the slot each mark actually has, so on-mark values scale with the room instead of defaulting to a fixed small point size. Both are advisory: the tool sizes the box, the selector/build owns the chart choice. When horizontal-bar category names are too long to sit in a sane left band it returns `wrap_y_labels_chars`, the per-line character width to wrap the category factor to (`str_wrap(labels, width = wrap_y_labels_chars)`): the left band is capped at a fraction of the panel width and the overflow becomes stacked text rows in a taller slot rather than an ever-wider margin that starves the panel (`0` = the names fit, no wrap). Sizing is one rule - each axis needs `slots x per_slot_floor` px, a continuous axis takes a pleasant aspect, faceting multiplies via an aspect-fill grid (each facet row also reserves its heading strip, at the lines `longest_facet_label_chars` needs when a heading wraps to its panel, and the frame's edge margin is reserved once), `y_slots` grows height while `x_slots` grows width and then triggers rotation. The grid is chosen so the whole image lands near `target_aspect` (the source image's width:height when repairing; default the delivery profile's aspect, 16:9 for chat) given each panel's floored shape - iterating candidate row counts, taking the tightest (compact, no empty sprawl) column count for each, and keeping the grid whose actual image aspect is closest to the target (tall panels take more columns, wide panels more rows) - and nothing is ever squashed below its legibility floor: when the content needs more room than the profile, *both* width and height are grown - the whole image is resized up - rather than clamped. A structured `fit` object reports `status` (`ok`/`over_ceiling`), `legible`, `over_width`/`over_height`, required vs ceiling dims, the achieved `min_panel_height_px`, and ranked machine-readable `directives` (`reduce_slots`/`reduce_panels`/`split_pages`/`drop_group`) so a caller branches on a boolean instead of parsing prose. It also returns `font_pt`, the canvas-scaled house font sizes per role resolved for the final canvas (identical to what `reserve_frame` uses), and reserves its text bands at those sizes so the height stays honest on a grown canvas; on-mark value labels stay the separate slot-driven `recommended_data_label_pt`. No regime enum, no count thresholds. For a heterogeneous layout - an aggregate/overview panel set apart from a small-multiple detail grid (the aggregate-and-parts guardrail) - pass `panel_groups` (`{role, n_panels, emphasis?, filled_marks?, x_slots?, y_slots?}` per group); each group is sized as its own sub-grid and stacked as a full-width band whose height follows what it has to show (a band of discrete rows takes its rows plus the axis expansion, so a one-bar overview above a four-bar detail gets about a third of the height, not half); the tool picks each group's column count against the same aspect target, and the per-band structure comes back as `regions` for the renderer to place instead of one uniform grid.
...
| `dataviz_mcp/scaffold.py` | Chart source written from the plan with one marks slot, and the slot check on the built plot (`scaffold_chart`, `check_chart`) |
| `dataviz_mcp/plot_data.py` | Mechanical tidy plotting frame from a role map - column whitelist and one shared order (`prepare_plot_data`) |
