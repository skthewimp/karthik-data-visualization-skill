...
With the dataviz MCP, the chart source starts from `scaffold_chart`: it writes the settings the plan has already decided - number format, palette, scales, facet grid, titles, fonts, margin, theme, and the value axis dropped once the planned value labels carry the reading - and leaves one `chart_marks` slot for the geoms and labels. The model writes only that slot and runs `check_chart`, which restores the scaffold and names any slot deviation (an override, a hand-picked hue, undersized text, missing promised labels). Without the tool, the same settings are applied by hand, exactly as the tools returned them.
