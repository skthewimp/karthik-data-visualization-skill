...
With the dataviz MCP, the chart source starts from `scaffold_chart`: it writes the settings the plan has already decided - number format, palette, scales, facet grid, titles, fonts, margin, theme, and the value axis dropped once the planned value labels carry the reading - and leaves one `chart_marks` slot for the geoms and labels. The model writes only that slot and runs `check_chart`, which restores the scaffold and names any slot deviation (an override, a hand-picked hue, undersized text, missing promised labels). The frame decides what a mark is: a segment or range is drawn between its `start` and `end`, a number printed beside a mark but not drawn (a growth rate on a revenue bar) prints through its own formatter as a note past the bar's end, and an overview set apart from its detail is drawn as its own region under one page frame. Without the tool, the same settings are applied by hand, exactly as the tools returned them.
