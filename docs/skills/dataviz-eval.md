...
The report also records five general release checks: visual integrity, relationship traceability, spatial economy, encoding semantics, and delivery robustness. These ask whether elements collide, whether the chosen identification system fits the chart and each label's bond with its mark is perceptually stronger than competing relationships, whether whitespace supports hierarchy, whether salient encodings have a recoverable job, and whether the exact export survives delivery. Direct labels, axes, legends, grouping, and small multiples are alternatives chosen from density and geometry; none is mandatory. Shared-row alignment alone does not bridge blank space. The checks do not prescribe a chart type, palette, fixed margin, or density threshold.
...
After user feedback, the evaluator turns the correction into a literal acceptance check. It verifies that requested removals are absent, requested additions are present in the right place, every expected category appears once, and legend swatches match the plotted marks. A general "looks clearer" inspection cannot clear these checks.
...
Colour evaluation checks whether colour has a job, whether the scale fits the data, whether mappings remain stable, whether saturation creates the intended hierarchy, whether adjacent colours stay distinct from one another and the background, and whether the chart still works in grayscale and after chat compression. WCAG-style ratios guide text and small-mark contrast without forcing every large fill into a mechanical threshold.
