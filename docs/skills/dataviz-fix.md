...
1. **Critique the source once.** Run `dataviz-critique` in the current chat as a single pass (JSON is fine), with no maker-checker on the critique. Judge the right form, the trifecta, whether the chart conveys its message (including the semantic scan), the requested style, and repair-versus-redesign with a bias toward redesign. Critique also decides, as a judgment call, the key messages the chart must carry, the content each message needs, and any source information dropped as *not* key (named explicitly, with a reason). In parallel, infer the raw data from the image thoroughly - the full period-by-category table (a value for every time period and every category/series/stack the chart encodes), not just totals or the envelope, so the key-message judgment has everything to weigh.
2. **Reconstruct.** Rebuild from the critique, prompt, inferred data, and inferred style. Carry every key message the critique named, showing the content each requires - which may mean more than one chart (a whole-and-parts split, a totals view alongside a per-category breakdown); the chart count, decomposition, and form are decided here. The form may change freely; what must survive is the messages, not every mark. Do not silently drop anything the critique judged key, and honour its explicit decisions to drop information judged not key. This is owned by the critique and this rebuild step - not something the later eval is expected to catch. Load `dataviz-selector` by default (unless the form is clearly correct), `karthik-data-visualization`, and the installed writing or brand-style skill when available. Invoke `chart-annotations` whenever the chart may have a point worth marking and let it judge whether any mark is warranted - it can decide the chart stays unmarked. Compose the headline and subhead here: title claim from `chart-annotations`, style from `karthik-data-visualization`, voice from the writing skill if installed (there is no dedicated headline skill). Honour every prompt constraint. Build one PNG, SVG, or PDF.
