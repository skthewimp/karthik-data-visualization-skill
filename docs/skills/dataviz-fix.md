...
`dataviz-fix` is the **repair orchestrator**, and it runs as an ordered sequence of stages - **diagnose+extract -> select -> build -> refine**. Separate calls per stage is the default and the right way (each call carries only that stage's skills plus a compact artifact handed forward; loading every skill into one context rots it, and the build stage has no use for the discovery or evaluation skills), but the stages are mandatory whether or not a driver splits them: a single-turn run still walks every stage in order and may not skip one on the assumption another call did it. The only stage any mode skips is `select`, and only for a literal `bounded-edit`. The eight steps below map onto those four stages (1-2 diagnose, 3 select, 4 build, 5-7 refine). Handoffs are structured text (markdown sections plus, at the branch points, a small `routing` block of `key: value` lines), not strict JSON, so the pipeline runs on cheaper / open-weight models too; the routing parser (`dataviz_mcp.handoff`) also accepts a JSON object. The content contract - the exact skill subset and required fields per stage - is `dataviz_mcp/stage_contracts.py:REPAIR_PIPELINE`; the skill carries the reasoning, the module carries the shape.
