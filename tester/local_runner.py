...
Choose the form cold: the source chart's form gets no vote. Set `builder` to `table` when the intent is exact lookup or the values are not commensurable on one scale, otherwise `chart` - this decides which builder skill the build stage loads. Set `needs_annotations` from whether the insight stage emitted any candidate annotation that survives its value-add bar - default it false when that list is empty, since most charts carry the claim in the headline and direct labels and need no on-chart marks; do not set it true merely because the form could host a callout. Set `needs_explainer`, `needs_color_plan`, and `needs_precision_plan` from whether the plan genuinely calls for accompanying prose, a colour plan, or a precision plan. Produce the design, the layout plan under the declared delivery condition, and an observable acceptance check for every fatal or major problem and every preservation requirement.
...
Python, R, Matplotlib, ggplot2, ragg, NumPy, Pillow, and this repo's `dataviz_mcp` package may be available. Use `render_and_inspect_chart(..., renderer="auto")`; it must choose ggplot2 for a supported R builder when the probe succeeds. For a table build, render the gtable through the same path with `content="table"`. Use Matplotlib only for an explicit requirement or a recorded unavailable/unsupported ggplot2 reason. In either renderer, define the visible design deliberately. Before the first render, pass every reader-facing text block whose words and anchor are already known to `recommend_text_placement` and apply its wrapping and placement; derive the trigger from the blocks, not a separate optional flag. For every series/category, on-mark data, and axis label, decide `max_width_px` and `max_lines` from the delivery condition, density, and available region. Set `allow_curtail: true` only when the intact `full_text` will appear in a compact key/footnote; otherwise revise an over-budget label's layout, wording, or form. Keep coordinate systems separate: data scales span the data's plotted extent (fitted to the data via `recommend_axis_range`, not the measure's natural domain - a percentage is not 0-100 unless the data reaches it), while non-data content and its whitespace live in layout/screen coordinates. Never change a quantitative scale merely to reserve room for non-data content, and never reserve the same room in both coordinate systems. Write the complete bundle into `{case_dir}` with artifact name `{candidate_path.name}`. An unchanged or perceptually unchanged artifact cannot satisfy an active correction. Copy an artifact unchanged only when no active correction or unresolved evaluator action requires a change. Stop after the inspected PNG exists at {candidate_path}."""
