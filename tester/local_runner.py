...
Python, R, Matplotlib, ggplot2, ragg, NumPy, Pillow, and this repo's `dataviz_mcp` package may be available. Use `render_and_inspect_chart(..., renderer="auto")`; it must choose ggplot2 for a supported R builder when the probe succeeds. Pass explicit width and height selected from the comparison, label geometry, and requested delivery conditions; a profile supplies DPI only. Use Matplotlib only for an explicit requirement or a recorded unavailable/unsupported ggplot2 reason. In either renderer, define the visible design deliberately. Write the complete bundle into `{case_dir}` with artifact name `{candidate_path.name}`. An unchanged or perceptually unchanged artifact cannot satisfy an active correction. Copy an artifact unchanged only when no active correction or unresolved evaluator action requires a change.
"""
