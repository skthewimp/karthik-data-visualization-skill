...
- Made colour count explicit and encoding-driven. Added `design.colour_groups` (integer) to `SELECT_SCHEMA`: the number of distinct colours the *encoding* needs, set by the select stage, not derived downstream from category count. Five series are five colours as five lines in one panel but zero as small multiples (the facet carries identity), one under focal-plus-grey, zero when direct labels or position carry identity. `needs_color_plan` is now gated on `colour_groups >= 1`, and `dataviz-color` reads `colour_groups` as its `n_series`.
