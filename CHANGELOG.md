...
- **`place_on_marks` never raises on label input.** No transform, or a label whose data position
  is missing, non-numeric or outside a log/sqrt scale, now comes back in a new `unplaced` list
  (`id`, `text`, `role`, `anchor_data`, `reason`) for the builder to draw at its mark; the rest
  are placed as usual. A wrong or ambiguous `mark_id` is listed in `unverified_attachments` with
  a warning instead of refused. A label without `max_width_px`/`max_lines` wraps to the
  annotation width and two lines with a warning. A label with no `id` gets `label-<n>`.
...
slots/panels scaled below their own floors - a sub-floor thumbnail returned as a clean fit,
with the truth buried in a warning string a weak maker skips. Three fixes:

- **No squash on either axis.** Nothing is ever scaled below its legibility floor to force a
...
- **Follow-up: geometry floor lowered from an 80% share to two labelled marks per panel.** The share was still too conservative. On a linear axis two labelled marks fix the pixel->value mapping, so the reader can recover every other mark and the ticks are duplicate ink. The floor is now an absolute count (`_REDUNDANT_AXIS_MIN_LABELS = 2`), not a ratio: a panel qualifies once at least two of its marks carry a value (or all of them, for a one- or two-mark panel). A single labelled mark still keeps the axis (one label cannot fix the scale). `dataviz_mcp/inspection.py`; fixtures/tests (`bars_two_labelled` fires, `bars_few_labelled` stays silent) in `dataviz_mcp/tests/`; wording synced in `dataviz-execution` (both) and `docs/mcp.md`.
...
- **A value on its own mark is no longer a self-collision.** The inspector already exempts role `data_label` from the text-mark collision check; the false positives came from on-mark values arriving as role `label`, which is flagged (a `label` overlapping a mark it does not name is a real accidental overlap). The inspector now also counts `data_label` (not only `label`) as a mark's direct value label, so direct-label coverage and the redundant-value-axis check still fire either way (`dataviz_mcp/inspection.py`). Each renderer supplies the role its own way: on matplotlib the builder tags an on-mark value with gid `data_label:<id>` (documented in `dataviz_mcp/README.md`); on ggplot there is no gid to set, but an in-panel `geom_text`/`geom_label` is data-driven by construction, so the adapter tags it `data_label` outright (`dataviz_mcp/rendering.py`) - no geometric guessing, and the collision check keeps its meaning on matplotlib where intent is declared.
...
- **`place_on_marks` returns native data coordinates for every movable label (`dataviz_mcp/text_fit.py`).** It inverts its own `data_to_pixel` affine (and each axis' `x_trans`/`y_trans` scale transform) to emit `placed_data` (where the label goes), `anchor_data` (the mark), and `leader_line_data` (`{from, to}` - the box-edge end and the mark end) when a leader is drawn. The builder draws the label and any connector from exact coordinates instead of guessing an `xend`/`yend` that misses the mark and cuts through whatever label lies between; the leader terminates at the bounding-box edge and the mark. A singular affine omits the data coordinates rather than fabricating them.
- **`plot_area` local correction on `place_on_marks` / `recommend_text_placement`.** A movable label left straddling the plot boundary is a clip canvas growth cannot fix (the box is inside the canvas, across the *plot* edge). Given the panel rectangle, the tool nudges it wholly inside and returns the exact `plot_boundary_correction` `{dx, dy}`, so the driver applies one specific move rather than re-placing everything. A label wholly inside, or a deliberate margin annotation wholly outside, is left alone; a corrected box that would collide is not silently shoved - it escalates.
- **`inspect_rendered_chart` classifies every defect and emits a `correction_plan` (`dataviz_mcp/inspection.py`).** Each defect carries a `defect_class` - `canvas` (grow it out), `placement` (an exact geometry-tool move), or `semantic` (a model judgement) - from a single code→class map keyed by every defect code, so a new code without a class fails a test instead of defaulting silently (the audit caught `TEXT_TEXT_COLLISION` and `HIERARCHY_TEXT_COLLISION`, previously unmapped). The report groups the defects into a `correction_plan` of those three classes; the `canvas` group carries the shared `growth_vector` (the same `suggested_dims`, null when nothing can grow), so "refit only when a growth vector exists" is a null check, not a judgement.
- **`dataviz-execution`, `dataviz-construct`, and `chart-annotations` (all copies) route on the classes.** The execution gate and the construct build/execution stages now read `correction_plan` and route by class - refit `canvas` (only on a non-null vector), apply the exact `placement` move through the geometry tools, escalate only `semantic` to a model patch - and draw leaders from `leader_line_data` / `placed_data` rather than improvising a segment. `chart-annotations` routes callouts through `plot_area` and the returned data coordinates too.
...
- **`dataviz-execution` (both copies).** The published post-render gate now teaches the refit-first loop too: it had lagged behind the stage contract, still instructing the model to "apply the suggested dims and re-render" by hand. It now settles resizable geometry with `refit_chart` first and reserves the flag vocabulary (`suggested_dims`, `overflow_px`, `separation_needed_px`, panel heights) for the residual the resize loop cannot fix.
...
- **`chart-annotations` (both copies).** The placement authority now routes to the forward tools too, and its by-eye rules stopped contradicting the separate-coordinates rule. Its Placement section had lagged: it taught only by-eye offsetting and told the author to make room by *extending the axis limits in the direction the text runs* - the exact scale-stretching `dataviz-construct`'s "keep coordinate systems separate" rule forbids. It now settles annotation geometry with `reserve_frame` + `place_on_marks`/`recommend_text_placement` where the harness has them (degrading to the same principles by eye where it does not), and reserves text room in the margin (or via `reserve_frame`), never by stretching the data scale - fixed in the placement bullets and the troubleshooting table.
- **`place_on_marks`: the no-transform error stopped lying about what ggplot emits.** The `ValueError` raised when no transform is supplied still listed `coord_flip`, non-identity scales, and faceted plots as cases the ggplot render "emits none" for - the exact cases the two commits below *added*. A driver hitting it would fall back to ggrepel for charts `place_on_marks` now handles. The message now names only the genuinely unsupported cases (non-Cartesian `coord_trans`/polar/sf, unreproducible date/logit/custom scales) and calls coord_flip / log-sqrt-reverse scales / facets supported, matching the adapter, `docs/mcp.md`, and `dataviz-construct`. Regression-guarded in `test_place_on_marks.py`.
- **`place_on_marks` now works on the ggplot2 path, not just matplotlib.** The ggplot adapter emits a `data_to_pixel` affine per panel, verified to land on a captured mark to the pixel. Coverage is any `CoordCartesian` plot: `coord_flip` (the affine carries cross terms - the flip maps x to the vertical axis, y to the horizontal), log/sqrt/reverse scales (each panel carries its `x_trans`/`y_trans`, which `place_on_marks` applies before the affine since the affine lives in scale-transformed space), and facets (one transform per panel, keyed to the panel's `axes_id`, so free scales resolve per panel). A non-Cartesian coord (polar/`coord_trans`/sf) or an unreproducible scale transform (date/logit/custom) emits none on purpose, and `place_on_marks` refuses without one (guarded) so a driver falls back to ggrepel rather than projecting through a wrong map. Also fixed: the transform was silently skipped when a builder returned `list(plot=, metadata=)` instead of a bare ggplot.
- **`fix`: ggplot rect marks are anchored by their real justification.** The adapter's rect reader assumed centre-anchoring, but ggplot's `GeomRect` draws at `(xmin, ymax)` with `just=c("left","top")`; the mismatch shoved each bar up by half its height (taller bars drifted higher, the tallest reported a negative top) and left by half its width, corrupting every mark box downstream geometry relies on. Now reads the grob's `just`/`hjust`/`vjust`. Regression-guarded with a vertical-column fixture (shared baseline, no negative tops, data-ordered heights, even spacing).
...
- **Labels use a judged readable measure, not a canvas fraction.** For every series/category, on-mark data, and axis label, build must pass `max_width_px` and `max_lines` based on the delivery condition, density, and available region. The existing 32%-of-canvas allowance now applies only to free annotations. Curtailment is opt-in (`allow_curtail: true`), retains `full_text`, and requires a compact key/footnote; otherwise an over-budget label stays intact and is reported for layout, wording, or form revision.
...
- **The distinction, `chart-annotations` and `karthik-evidence-builder` (both copies each).** An annotation carries a fact that is *not in the dataset* - a rainy day behind a spike, a regulation, an acquisition, a change of government - which the chart's encoding structurally cannot draw. Anything already in the data ("peak", "all-time high", "X -> Y", a rank, a trend, a crossover) is a **direct label**, or it is nothing - never an annotation. The whole "mark the interesting data feature, then filter" apparatus (candidate inventory, concentration check, value-add gate, significance ladder, aggregate/difference clauses) is deleted, not extended.
- **The bar is self-enforcing.** An external fact cannot be obtained by studying the data harder - it comes from the brief, the domain, or the source, or it does not exist. So the default is no annotation and most charts stay unmarked, with nothing to rationalise: a fact is external or it is not. A cause invented to fill the slot is worse than a blank chart; a suspected link is worded as coincidence in time, never established cause.
- **A direct label is one mark's value, not a change - and only the few marks that carry the point.** Guards the other side of the swing: reclassifying in-data callouts as "labels" must not licence a wall of 200 stamped values, nor "X → Y" / "+38%" / "doubled" change-narration on marks. A change is the shape the chart already draws; its claim goes in the title, in words. A label carries a single point's value, on endpoints / the extreme / the mark the claim rests on. New mistake rows and label-section wording in `chart-annotations`; the evidence and `karthik-data-visualization` copies say the same.
- **Contract, `dataviz_mcp/stage_contracts.py`.** `candidate_annotations` items become `external_fact` + `anchor` + `source` (all required), replacing the self-justifying `why_it_clears_the_bar`. The idea-gate and `needs_annotations` prose realigned. `karthik-data-visualization` (both copies) reworded to send in-data findings to direct labels and reserve annotations for outside facts. Docs (`docs/skills/chart-annotations.md`, `docs/skills/karthik-evidence-builder.md`) rewritten. Contract suite 43 pass.
