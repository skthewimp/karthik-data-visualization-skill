...
description: Choose how many significant digits to show for chart and table numbers - keyed to the spread (max-min), not individual values, one uniform place per column. Never fabricate precision.
metadata:
...
- **No approximation glyphs on a value that is known exactly.** A plotted datum you hold precisely is labelled as itself - `610`, not `≈610` or `~610` or `about 610`. The `≈` asserts an uncertainty the data doesn't have, and it also breaks the redundant-axis check (a tick reader matches label text to the mark value, and `≈610` no longer reads as `610`), so the value axis it should have retired stays on the chart. Reserve `≈`/`~` for a figure that is genuinely an estimate (a rounded external headline, a read-off from a source chart), never for a value from the data in hand.
