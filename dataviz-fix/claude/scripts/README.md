...
Example after Hermes records a user-turn grant:
...
The Hermes release guard treats only `MEDIA:/absolute/path` as a delivery attempt. Plain image/PDF paths in stopped-run status reports are diagnostic and leave the useful response intact; an explicit `MEDIA:` attachment remains fail-closed unless the exact turn-bound artifact has an independent `Send` and matching hash.
