...
Preserve every valid piece of information the source carries. Redesigning the form is encouraged - small multiples, lines, whatever the critique picks - but the new chart must keep every category, series, period, unit, and qualification the original encoded. Dropping any of them is a decision the prompt or the critique must justify, never a side effect of choosing a new form. Collapsing a ten-category stacked chart into a single total is a failure, not a simplification. Preserving the source's information is the job of this step and the critique before it - not something a later reviewer is expected to catch.
