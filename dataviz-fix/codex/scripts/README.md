# Optional audited dataviz-fix case manager

Do not invoke this from the default chart-repair path. `case_manager.py` records the original chart, versioned context, revisions, `dataviz-eval` gate results, feedback, state transitions, budgets, usage, best candidate, acceptance, and skill diagnosis when the user explicitly requests an audited workflow.
