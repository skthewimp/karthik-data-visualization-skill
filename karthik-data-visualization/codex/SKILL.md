...
- Preserve an existing semantic palette unless changing it solves a stated problem. When colour encodes a series or category, every legend swatch, mark, direct label, and annotation must use the same mapping.
...
- After changing an axis label, legend, or colour, inspect that exact element in the export. Confirm every required category remains identifiable and correctly bound to its marks, and every legend swatch matches its plotted marks.
