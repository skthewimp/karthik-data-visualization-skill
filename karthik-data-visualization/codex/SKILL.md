...
Surface the finding on the chart, not only in the title: where a single mark or point carries it, put a direct label - one mark's value - on the few points that carry the claim, never all of them. A change or comparison ("X to Y", "+38%", "doubled") is neither a label nor an annotation; the shape shows it, so its claim goes in the title. Reserve annotations for a fact from outside the data that explains the finding (a cause, an event, a regime change). When `chart-annotations` is loaded it makes that call and places both. When an installed writing or brand-voice skill is present in the environment, use it to word the headline, claims, subtitle, and annotation text; this skill sets their style and placement. If none is installed, follow the prompt's stated preferences and the title/subtitle rules below.
...
- Treat direct values and series names as a repeated labelling system, not as free callouts. Place each selected point value with one consistent small offset from its mark and each series name adjacent to its line endpoint (or another clear point on the line). Pass point values as fixed `data_label` blocks and series/category names as `label` blocks. Draw a connector only when the tool returns a `leader_line`; never add a dash or leader merely to decorate the label. When the anchors came from `place_on_marks` (labels tied to data marks), draw the label and connector from the native data coordinates it returns - `placed_data` (the label's box top-left; draw it left/top-anchored) and `leader_line_data` (the leader endpoints) - never a hand-written `geom_segment`, whose guessed endpoint misses the mark and runs through a neighbour. If several ordinary labels require leaders, revise the label set, anchor, or layout so they can sit adjacent instead of accepting a field of callouts.
...
- Fit the value axis to the data it carries; the unit's theoretical maximum is not the axis maximum. A percentage or share does not earn a 0-100 domain just for being a percentage - if the plotted values run 1 to 44, an axis to 100 spends half the panel on empty space and flattens every line into the lower band. Set the upper bound just above the largest plotted value (a small nice-number headroom), not at the unit's ceiling. Keep the baseline at zero when the encoding needs it (bars, or where zero is a meaningful reference the reader compares against) and when a line's absolute level matters; a non-zero baseline is legitimate for lines whose story is movement within a narrow band, as long as it is not disguising the magnitude of a change. Use the full 0-100 only when that full range is itself the point (a gauge against capacity, shares meant to be read against the whole).
- A shared or compressed scale buys comparability at the cost of resolution: any series sitting well below the axis maximum - a small series beneath a dominant one, a low panel in a shared-scale grid - flattens toward the baseline, so its level and change can no longer be read off the axis. When a value the reader needs cannot be resolved from the scale, recover it by putting the number on the mark - the endpoints, or the focal value - rather than leaving a flat line to be eyeballed. Reach for direct value labels before abandoning the shared scale that provides the comparison, or before adding a second scale that breaks it. But labelling has a limit: when several small series converge into a baseline cluster, their labels collide as fast as the lines do, and stamping a value on every point makes the pile worse, not readable. That is the signal to change the form, not to add more labels - split the small series into their own panel or inset at a scale that separates them, or move them to a table - and never resolve a convergent cluster by labelling every point of every line.
