...
3. **Reserve and apply the frame.** Pass the finalized frame text and design to `reserve_frame`. The renderer lays out the chrome (title, subtitle, axis, legend, caption) natively inside the sized canvas: set `plot.margin` to the returned `plot_margin_px` (the outer edge alone) and **never derive margins from the reserved bands** - summing a band into the margin reserves that chrome twice and collapses the panel. Carry its `frame_blocks` forward as obstacles and use `plot_area` as the label boundary; both are advisory, not a margin.
...
When using the MCP renderer, pass the frame result as `inspection_contract.frame` and the returned placements as `inspection_contract.placements` to `render_and_inspect_chart` and `refit_chart`. The inspector verifies their text and bounds without distinguishing ggplot `label` from `data_label`. Supply the intended rendered `mark_id` to `place_on_marks` when known from the same transformed data; it rejects anchors off that target. Disclose `unverified_attachments` when the target is unavailable; never infer intent from proximity or adapter-assigned text roles.
...
- **Claim-first title; a subtitle only when there is a second message.** Title states the finding. The subtitle exists **only** to carry a trend or a second comparison the title can't hold; a static snapshot or a plain ranking has no such message, so it gets **no subtitle** - an empty subtitle slot is not filled with units, denominator, or scope caveats (those belong in the caption/footer). Neither line restates mechanics. Fall back to a question, measure, or explicit null only when the evidence genuinely won't support a claim - never manufacture one, and never manufacture a subtitle.
...
- **Bar value labels default inside the bar, at its value end.** Print the value inside the bar near its filled end, not floating past it - that keeps the number bound to its mark and spends no extra ink. Fall to *outside* the end only when the bar is too short to hold the label (compute it: does the rendered label width plus padding fit the bar's pixel length? inside if yes, outside if no) - decided per bar, so a long bar labels inside while a short one in the same chart labels outside. Size the value from the room it has (`recommend_layout`'s `recommended_data_label_pt`, derived from the slot), never a fixed small point size. Colour follows the surface it sits on (see next rule).
...
- Probe before generating chart source. With the MCP's automatic selection, use R whenever `Rscript`, `ggplot2`, and `ragg` are available; generate Python only when that backend is unavailable. Do not switch to Python after an R build error. `auto` does not translate existing source or treat a `.py` file as a reason to bypass available R.
