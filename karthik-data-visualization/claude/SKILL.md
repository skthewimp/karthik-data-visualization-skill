...
Use this skill to implement the visual system of a chart: hierarchy, identification, typography, colour, spacing, graphical integrity, and rendered-output inspection.

Do not use it to choose the analytical question, chart form, annotation candidates, critique format, or release verdict. Those belong to `karthik-analysis-planner`, `dataviz-selector`, `chart-annotations`, `dataviz-critique`, and `dataviz-eval` respectively.
...
2. Use the structure chosen by `dataviz-selector`; if the form is still open, invoke that skill before styling.
...
- Apply the identification system consistently across every applicable panel, facet, series, or repeated structure. Each instance must remain interpretable unless one shared identifier serves all instances equally well.
