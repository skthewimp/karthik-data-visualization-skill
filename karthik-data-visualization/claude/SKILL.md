...
6. **Reserve the frame before the first render.** Placement is settled by measurement, not discovered by clipping. Every block that lives in the margins - title, subtitle, footer/caption, axis titles and ticks, legend - is sized and given its own pixel band first, and the marks fill only the plot rectangle that remains. Where it sits does not depend on the data, so it is fixed *before* rendering, blind. Where the harness provides them, `reserve_frame` (and `recommend_layout` for the canvas) does this arithmetic and returns the `plot_area` the marks may fill; where it does not, compute the margins explicitly and confirm by eye - never draw marks edge-to-edge and trust the chrome to fit. This is the deterministic path: a clipped title or a header colliding with the plot is a first-pass reservation you skipped, not a revision you owe.
7. **Render and inspect the export - the safety net, not where placement is decided.** The frame and data-glued labels were already reserved at step 6; this pass only confirms the pixels and catches what measurement could not. Fix anything still clipped, overlapping, or misaligned from the actual image, then re-render. Confirm the House defaults below. Once defect-free, read the whole image as a composition (`dataviz-aesthetic`): one focal element seen first, every mark earning its place, whitespace grouping not filling. A defect-free chart can still read as styled-default; the composition pass is what makes it premium.
