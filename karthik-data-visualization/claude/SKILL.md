...
- Prefer small multiples to crowded multi-series panels when comparison across groups is the task. Lay the panels out as a grid (rows x columns) sized to the delivery medium, not a single-column or single-row strip; a tall stack of many panels defeats the side-by-side comparison. Order panels by a meaningful key (magnitude, cluster) rather than input order, and mark per-panel scales when they are free.
