...
- **Preserving the message is not preserving the form.** The data must survive; the *encoding* must not, and often should not. Keeping the source's chart type is not preservation - re-rendering the same form more cleanly fails the message if that form was what made it hard to read. The textbook case: a many-series stacked bar or area where the message is per-series comparison or each series's trajectory. No reader can follow one model through a ten-deep stack, so a tidier stack - legend moved, canvas whitened, colours retained - still fails. The right move is a form where each series is legible: small multiples, direct-labelled lines, a ranked or indexed view. When the source form is the reason a key message is hard to read, changing the form *is* the repair; reproducing it is the bug.
