...
16. **`karthik-table-style`** - table-craft rules for when the chosen form is a table: emphasis as scarce ink, decimal-point alignment, precision keyed to variance, content-sized columns, minimal rules, tabular figures, and conditional formatting by default (data bars, shading, sparklines, focal emphasis) drawn by the table tools.
